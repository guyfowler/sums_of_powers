...
************************************************************************************
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
PARI scripts for Section 8.4 in
...
Contents
    ...
    Scripts on discriminants
        ...
        discs(n)
        find_discs_cl_between(n, k, r)
        ListTwoElemSameField
        ListExceptionalPairs

    Scripts for 2-elementary discriminants

        rho_two(n)
        disc_is_two_elem(n)

    Eliminating the remaining cases

        test_trip(triple)
        test_all_trips(v)

    Finding the triples of discriminants to check
...
Input: integers n, k, r>0
Output: the list of all discriminants Delta with abs(Delta) \leq n and k \leq h(Delta) \leq r
-----------------------------------------------------------------------------*/
...
find_discs_cl_between(n, k, r)={
    ...
    if(type(r)!="t_INT", print("Input error"); return);
...
output: vector v such that v[i] is the list of discriminants in the ith row of Table 2 in "CM-points on straight lines", by B. Allombert, Yu. Bilu, & A. Pizarro-Madariaga, in Analytic Number Theory, Springer, 2015, pp.1-18.

See Proposition 7.5
-----------------------------------------------------------------------------*/

ListTwoElemSameField=[[-3, -4, -7, -8, -11, -12, -16, -19, -27, -28, -43, -67, -163],[-24,-32,-64,-88],[-36,-48],[-15,-20,-35,-40,-60,-75,-100,-115,-235],[-52,-91,-403],[-51,-187],[-96,-192,-288],[-180,-240],[-195,-520,-715],[-120,-160,-280,-760],[-340,-595],[-480,-960]];

/*------------------------------------------------------------
ListExceptionalPairs

output: list of all pairs (Delta_x, Delta_y) satisfying (7.1)
...
input: a vector of discriminants [Delta_x, Delta_y, Delta_z] satisfying one of the exceptional cases (1)(a)--(2)(c) listed in Section 8.4
output: 1 if the set {1, x, y, z} is linearly independent over Q for all singular moduli x, y, z of respective discriminants Delta_x, Delta_y, Delta_z; 0 otherwise
------------------------------------------------------------------------------*/

test_trip(triple)=
{
my([Delta_x, Delta_y, Delta_z]=triple);
my(f_x,f_y,f_z,L,x,y,z,G,n,u,v,w,m,b, t, e, k);

f_x=polclass(Delta_x,0,'y);
f_y=polclass(Delta_y,0,'y);
f_z=polclass(Delta_z,0,'y);

if(disc_is_two_elem(Delta_y)==0,
L=subst(rnfequation(polredbest(f_y),'x^2-Delta_y),'x,'y),
L=f_y);

L=nfinit(polredbest(L));
x=nfroots(L,subst(f_x,'y,'x));
y=nfroots(L,subst(f_y,'y,'x));
z=nfroots(L,subst(f_z,'y,'x));

e=nfalgtobasis(L, 1);
u=nfalgtobasis(L, y[1]);

m=0;
k=0;

for(i=if(Delta_y==Delta_z, 2, 1), length(z),

v=nfalgtobasis(L, z[i]);

for(j=if(Delta_y==Delta_x, 2, 1), length(x),

w=nfalgtobasis(L, x[j]);

t=[e, u, v, w];

a=lindep(t);

if(length(a)==0, m=m+1, print(a));
k=k+1;
...
input: a vector v, each entry of which is a vector [Delta_x, Delta_y, Delta_z] of discriminants satisfying one of the exceptional cases
output: 1 if the set {1, x, y, z} is linearly independent over Q for all singular moduli x, y, z of respective discriminants Delta_x, Delta_y, Delta_z for every [Delta_x, Delta_y, Delta_z] in v; otherwise prints a list of those [Delta_x, Delta_y, Delta_z] which fail and returns 0
--------------------------------------------------------------------------------*/
...
test_all_trips(v)={
    ...
    if(test_trip(v[i])==1, k=k+1, print(v[i]));
    
    );

    if(n==k, return(1), return(0));
}
...
output: list of all triples (Delta_x, Delta_y, Delta_z) satisfying one of the conditions (1)(a)--(2)(c) listed in Section 8.4
----------------------------------------------------------------------------*/

find_triples()={
my(u, v, n, Delta, f, g, w, m, l, Delta_x, Delta_y, Delta_z);

u=[];

v=find_discs_cl_between(4075, 3, 6);
...
if(qfbclassno(4*Delta)!=qfbclassno(Delta), next());

f=polclass(Delta);
g=polclass(4*Delta);

if(nfisincl(f, g)==0,next()); 

u=concat(u, [[4*Delta, Delta, Delta]]);
...
u=concat(u, [[4*Delta, 4*Delta, Delta]]);
...
if(quaddisc(Delta_x)==quaddisc(Delta_y)&&quaddisc(Delta_x)==quaddisc(Delta_z), next());

u=concat(u, [[Delta_x, Delta_y, Delta_z]]);
...
if(Delta%16!=0&&Delta%16!=4, next());

if(2*qfbclassno(Delta)!=qfbclassno(9*Delta/4), next());

f=polclass(Delta);
g=polclass(9*Delta/4);

if(nfisincl(f, g)==0,next()); 

u=concat(u, [[Delta, (9/4)*Delta, (9/4)*Delta]]);
...
w=find_discs_cl_between(5879, 7, 8);
m=length(w);
...
w=find_discs_cl_between(1557, 9, 10);
m=length(w);
...
w=find_discs_cl_between(790, 11, 14);
m=length(w);
...
if(qfbclassno(Delta)>=5, next());

if(2*qfbclassno(Delta)!=qfbclassno(4*Delta), next());
...
u=concat(u, [[Delta, 4*Delta, 4*Delta]]);
...
w=find_discs_cl_between(304, 5, 6);
m=length(w);
...
if(qfbclassno(Delta_x)<3, next());

Delta_y=w[i][2];

u=concat(u, [[Delta_x, Delta_y, Delta_y]]);
